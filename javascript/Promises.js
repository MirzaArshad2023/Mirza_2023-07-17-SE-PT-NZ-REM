/*let myCustomPromise = new Promise((resolve, reject)=>{
        ...
        while(!success)
        {
            resultval = (Math.random() * 10 );
            console.log(`number generated: ${resultval}`)
            if(resultval%2 == 0)
            {
                resolve(`${resultval} is even number`)
                break;
            }
            else
                reject(`${resultval} is not an even number`)
        }
});

myCustomPromise.then((result)=> console.log(result))
